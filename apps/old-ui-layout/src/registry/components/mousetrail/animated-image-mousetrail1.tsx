...
export default function ImageMouseTrail3() {
  ...
  const handleOnMove = (e) => {
    if (distanceFromLast(e.clientX, e.clientY) > window.innerWidth / 20) {
      console.log(globalIndex, refs.current.length)
  ...
  return (
    ...
    >
      {items.map((item, index) => (
        <img
          key={index}
          className="object-cover z-10  w-40 h-48  opacity-0   data-[status='active']:opacity-100 transition-transform duration-500 data-[status='active']:ease-out-expo   absolute  -translate-y-[50%] -translate-x-[50%]"
          data-index={index}
