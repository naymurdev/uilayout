...
description: A responsive header that can be closed by dragging on mobile devices and functions as a standard dialog or drawer on desktop.
component: true
---
...
<Tabs defaultValue="preview" className="w-full relative">
  ...
  <TabsContent value="preview">
    <ComponentPreview name="headerDrawer" category="modal" />
  </TabsContent>
  <TabsContent value="codesnippet">
    <CodeWrapper noExpand={true}>
      <CodePreview name="headerDrawer" category="modal" />
    </CodeWrapper>
...
```ts title="you can use state or default button to control the dialog" {1,5-7}
